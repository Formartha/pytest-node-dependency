...
def get_dependency_nodeid(item, dependency):
    ...
    """
    base_item_nodeid = clean_nodeid(item.nodeid).rsplit('::', 1)[0]
...
def reorder_tests(items):
    ...
    """
    dag = nx.DiGraph()
    nodeid_to_item_map = {clean_nodeid(item.nodeid): item for item in items}

    for item in items:
        dag.add_node(clean_nodeid(item.nodeid))

        marker = item.get_closest_marker('depends')
        if marker is not None:
            for dependency in marker.kwargs['on']:
                dependency_nodeid = get_dependency_nodeid(item, dependency)
                dag.add_edge(dependency_nodeid, clean_nodeid(item.nodeid))

    without_deps = [item for item in items if not dag.pred[clean_nodeid(item.nodeid)]]
    with_deps = []

    for item in items:
        if dag.pred[clean_nodeid(item.nodeid)]:
            dependency_nodeid = list(dag.pred[clean_nodeid(item.nodeid)].keys())[0]

            if check_skip_marker(dependency_nodeid, nodeid_to_item_map):
                current_nodeid = dependency_nodeid
                while dag.succ[current_nodeid]:
                    nodeid = list(dag.succ[current_nodeid].keys())[0]
                    if not check_skip_marker(nodeid, nodeid_to_item_map):
...
def pytest_runtest_setup(item):
    handle_failed_dependency(item)


def pytest_runtest_teardown(item):
    handle_failed_test(item)
